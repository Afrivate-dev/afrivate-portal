"""Branded DOCX companions for end-of-engagement letters."""
import json
...
OUT_DIR = ROOT / "hiring" / "end-of-engagement"
DATA = json.loads(
    (Path(__file__).resolve().parent / "content" / "end-of-engagement-letters.json").read_text(
        encoding="utf-8"
    )
)
...
def write_letter(letter):
    ...
    for section in doc.sections:
        ...
        r = fp.add_run(
            "hr@afrivate.org  ·  Confidential people document          AfriVate Technologies Ltd  ·  RC: 9210092"
        )
    ...
    para(doc, "End of Engagement", size=16, bold=True, align="center", space_after=2, all_caps=True)
    para(doc, DATA["kicker"], size=10, color=MUTED, align="center", space_after=12)

    meta = [
        ("To", letter["fullName"]),
        ("From", DATA["from"]),
        ("Effective", f"{DATA['letterDate']} — immediate"),
    ...
    para(
        doc,
        "Status: Portal and Slack access are revoked with immediate effect. This letter does not create employment or any right to pay.",
        size=10,
        color=MUTED,
        space_after=12,
    )
    para(doc, f"Dear {letter['firstName']},", size=11, bold=True, space_after=8)
    para(doc, DATA["notice"])
    para(doc, DATA["groundsIntro"])
    for item in DATA["grounds"]:
        bullet(doc, item)
    para(doc, DATA["surviving"], space_before=6)
    para(doc, DATA["financial"])
    para(doc, DATA["closing"])
    para(doc, "Yours faithfully,", space_before=10, space_after=18)

    sig = doc.add_table(rows=1, cols=2)
    no_table_borders(sig)
    left, right = sig.rows[0].cells
    set_run(left.paragraphs[0].add_run("Emmanuel Okpiaifo"), size=11, bold=True)
    for line in ["Human Resources Manager", "AfriVate Technologies Ltd"]:
        p = left.add_paragraph()
        p.paragraph_format.space_after = Pt(0)
        set_run(p.add_run(line), size=10, color=MUTED)

    set_run(right.paragraphs[0].add_run("For AfriVate Technologies Ltd"), size=11, bold=True)
    p = right.add_paragraph()
    p.paragraph_format.space_after = Pt(0)
    set_run(p.add_run("People & Culture"), size=10, color=MUTED)

    para(doc, DATA["footer"], size=9, color=MUTED, space_before=16)

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out = OUT_DIR / f"Afrivate-End-of-Engagement-Letter-{letter['slug']}.docx"
    dl = Path(rf"C:\Users\DELL\Downloads\Afrivate End of Engagement Letter - {letter['fullName']}.docx")
    doc.save(out)
    copyfile(out, dl)
    print("saved", out)
    print("copied", dl)
