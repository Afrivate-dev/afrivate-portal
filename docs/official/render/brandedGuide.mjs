/**
 * Shared branded HTML + PDF renderer for AfriVate official guides.
 */
...
<body>
  <div class="shell">
    ...
    <h1>${title}</h1>
    <section class="meta">
      <div><strong>Document Code</strong><span>${code}</span></div>
      <div><strong>Product</strong><span>AfriVate Team Space · portal.afrivate.org</span></div>
      <div><strong>Audience</strong><span>${audience}</span></div>
...
/**
 * @param {{ title: string, code: string, audience: string, lastUpdated: string, bodyFile: string, outBase: string, copyToDownloads?: boolean }} spec
 * @param {{ page: import('playwright').Page }} ctx
 */
export async function writeBrandedGuide(spec, ctx) {
  await mkdir(outDir, { recursive: true })
  const body = await readFile(path.join(__dirname, spec.bodyFile), 'utf8')
  ...
  const htmlPath = path.join(outDir, `${spec.outBase}.html`)
  const pdfPath = path.join(outDir, `${spec.outBase}.pdf`)
  await writeFile(htmlPath, html, 'utf8')
  await ctx.page.goto(`file:///${htmlPath.replace(/\\/g, '/')}`, { waitUntil: 'networkidle' })
  await ctx.page.pdf({
    path: pdfPath,
    format: 'A4',
  ...
  if (spec.copyToDownloads !== false) {
    const downloadsPath = path.resolve(`C:/Users/DELL/Downloads/${spec.outBase}.pdf`)
    try {
...
export async function renderBrandedGuides(specs) {
  const browser = await chromium.launch()
  const page = await browser.newPage()
  try {
    for (const spec of specs) {
      await writeBrandedGuide(spec, { page })
    }
  } finally {
    await browser.close()
  }
